...
async function hasStaffAccess(interaction) {
    ...
    return !!(
        ...
        member.roles.cache.has(guildConfig.bugStaffRoleId)
    );
}
...
function buildEmbed(bug) {
    return new EmbedBuilder()
        ...
        .addFields(
            {
                name: 'Original Title',
                value: bug.originalTitle,
                inline: false
            },
            {
                name: 'Status',
                value: `${STATUS_EMOJI[bug.status]} ${bug.status}`,
                inline: true
            },
            {
                name: 'Reporter',
            ...
            {
                name: 'Reported',
                value: `<t:${Math.floor(bug.createdAt / 1000)}:F>`,
                inline: true
            },
            ...(bug.resolvedAt ? [{
                name: '✅ Resolved At',
            ...
            collector.on('collect', async i => {
                ...
                if (
                    ...
                    try {
                        ...
                        client.logger?.warn?.(
                            `[BugTracker] Failed to update forum tag for ${bug.bugId}: ${tagErr.message}`
                        );
                    }
                                        ...
                                        .addFields(
                                            {
                                                name: 'Status',
                                                value:
                                                    `${STATUS_EMOJI[newStatus]} ${newStatus}`,
                                                inline: true
                                            },
                                            {
                                                name: 'Updated By',
