    ...
    async execute(interaction, client) {
        ...
        if (!await hasStaffAccess(interaction)) {
            return interaction.editReply({
                content:
                    '❌ You need staff or admin permissions to approve reopen requests.'
            });
        }
        ...
        if (bug.status !== 'Resolved') {
            return interaction.editReply({
                content:
                    '❌ This bug is not resolved; nothing to reopen.'
            });
        }

        // Reporter must still be a supporter
        const isSupporter =
            await Supporter.findOne({
        ...
        if (!isSupporter) {
            return interaction.editReply({
                content:
                    '❌ The reporter is no longer a premium supporter. Reopen denied.'
            });
        }
        ...
        if (
            !bug.resolvedAt ||
            Date.now() - bug.resolvedAt.getTime() >
                REOPEN_WINDOW_MS
        ) {
            return interaction.editReply({
                content:
                    '❌ The 24-hour reopen window has expired.'
            });
        }
        ...
        try {
            ...
            if (thread) {
                try {
                    ...
                    console.warn(
                        `[BugTracker] Failed to apply Received tag when reopening ${bugId}: ${tagErr.message}`
                    );
                }
            }
            ...
            if (thread) {
                const embed =
                    new EmbedBuilder()
                        ...
                        .setColor(0xe74c3c)
                        .setDescription(
                            `This bug report has been reopened by <@${interaction.user.id}>.`
                        )
                        .addFields(
                            {
                                name: '📊 Status',
            ...
            /* thread may no longer be accessible */
        }
        ...
        try {
            ...
            const dmEmbed =
                new EmbedBuilder()
                    ...
                    .setColor(0xe74c3c)
                    .setDescription(
                        `Your bug report \`${bug.bugId}\` has been reopened by staff.`
                    )
                    .addFields(
                        {
                            name: '📋 Bug ID',
